...
function lookupDescription(char, simplifiedMode) {
  ...
  if (simplifiedMode)
    ...
    return null //char + ': carattere non riconosciuto'
}
...
function toJson() {
  ...
  for (const item in CHARACTER_SIMPLIFIED) {

    const text = CHARACTER_SIMPLIFIED[item]
    simplified[item] = { 
      text,
      filename: audioFilename(text, { homedirectory: '', language: 'it'} )
    }
