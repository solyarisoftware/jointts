 ...
 * @return {Promise<ConcatAudioFileObject>} object returned by the ffmpeg spawned process, plus the attribute outputFilename
 */
async function concatAudiofiles( inputFilenames, outputFilename=fileNameConcat(inputFilenames) ) {
  ...
  const spawnResult = await spawnAsync('ffmpeg', args)
  
  const result = {...spawnResult, ...{outputFilename}}

  //console.info( result )

  // add attribute outputFilename to the object returned by spawnAsync
...
async function main() {
  ...
  try {
    const result = await concatAudiofiles( inputs /*, 'audio/it/concatexample.mp3'*/ )
