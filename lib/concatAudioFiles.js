...
function concatAudiofiles ( inputFilenames, outputFilename ) {
  ...
  // WARNING about ffmpegi behaviour: it seems that ffmpeg do not riase any error 
  // if one or more files to be concatenated are not in the same codec/ or if the files exists!
...
async function test() {
  ...
  const output= 'audio/concatAudioFiles.mp3'
