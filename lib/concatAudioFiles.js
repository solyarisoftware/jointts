...
const { fileExtension, fileName, filePath } = require('./fileHelpers')
 ...
 *
 */
function concatAudiofiles ( inputFilenames, outputFilename ) {

  const firstFile = inputFilenames[0]
  const suffix = fileExtension(firstFile)

  // if outputFilename is not present
  // is made joining names of inputFilenames
  if ( ! outputFilename ) {
    ...
    outputFilename = `${path}/${name}.${suffix}`
  }  
...
async function test() {
  ...
  //const output= 'audio/it/concatexample.mp3'
  console.log ( `\ninput files: ${inputs.join(', ')}` )

  concatAudiofiles( inputs /*, output*/ )
    .then( result => {
