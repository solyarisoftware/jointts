...
### Step 1: off-line speech files preparation

Following a configuration file, all audio files required must be produced, 
with a recording or a download.
...
- Voice-recording: prerecorded audio files, result of a voice actor recordings
...
```
                   +---------------------+
                   |                     |
text sentence ---> |  joinTTS run-time   | --->  audio file
                   |                     |
                   +---------------------+
                   |        ffmpeg       |
                   +---------------------+
...
with a bunch (40%) of features implemented. 
