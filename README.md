...
The speech is produced by concatenating prepared audio files sources, for letters, words, template literals, entire phrases. 
...
The startegy is to prepare "off-line" all audio files need, to be available at run-time for fast concatenative audio generation. 


## Internationalization 

Speech is language-dependent. The speech transaltion of any text depends on a specific natural language of reference.
...
Phrases built concatenating words and or letters i
(that's the common sense, general case of concatenative text-to-speech).
...
Example:
```javascript
const fileName = conca.ttsfile('Container JL1349-76 has been cleared for pick-up.', 'en', 'ogg')
```
